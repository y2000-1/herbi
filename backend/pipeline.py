"""
Unified inference pipeline for HerbiEstim — cross-platform edition.
...
No system-specific dependencies — works on Windows, Linux, macOS.
"""
...
class HerbiEstimPipeline:
    ...
    def _load_sam(self):
        ...
        try:
            ...
            logger.info(f"  Box threshold: {self.sam_box_threshold}")
            logger.info(f"  Text threshold: {self.sam_text_threshold}")
            ...
            logger.warning("  To use SAM, ensure: 1) model weights in model_saved/,"
                           " 2) GroundingDINO and segment-anything packages installed,"
                           " 3) CUDA is available (or set HERBI_SAM_DEVICE=cpu).")
            self._sam_segmentor = None
    ...
    def _encode_image_mobile(self, img: np.ndarray) -> str:
        ...
        Downscales large images and uses moderate JPEG quality to reduce
        mobile data usage while maintaining visual quality for leaf inspection.
        """
        h, w = img.shape[:2]
    ...
    def analyze(self, image_bytes: bytes, dpi: int = 300,
        ...
        Args:
            image_bytes: Raw image file bytes (jpg/png/tiff/bmp/webp).
        ...
        if debug:
            ...
            logger.info(f"  Input image size: {w}x{h}, channels: {original_img.shape[2] if len(original_img.shape) > 2 else 1}")
            ...
            if use_sam and sam_debug is not None:
                ...
                if boxes is not None and confs is not None:
                    logger.info(f"  GroundingDINO detected {len(boxes)} boxes, confidences: {[round(float(c), 3) for c in confs[:5]]}")
                ...
                filt_vis = render_filtered_masks(original_img, kept_masks, rejected_masks)
        ...
        if not standardized_leaves:
            logger.warning("No leaves detected. Possible causes: "
                           "image has no clear leaf, background is too complex for OpenCV mode, "
                           "or SAM is not enabled. Try uploading a clear leaf photo or enabling SAM mode.")
            return {
                'leaves': [],
                'summary': {
                    'num_leaves': 0,
        ...
        for i, (leaf_data, pred_data) in enumerate(zip(standardized_leaves, predicted)):
            ...
            damage_pct = max(0.0, min(1.0, float(damage['damage_pct'])))
            leaf_area = float(damage['leaf_area']) if is_scanned else None
            intact_area = float(damage['intact_area']) if is_scanned else None

            leaf_result = {
                'leaf_id': int(leaf_data['leaf_id']),
                'leaf_area_cm2': leaf_area,
                'intact_area_cm2': intact_area,
            ...
            damage_pcts.append(damage_pct)
            if is_scanned:
                total_leaf_area += leaf_area
                total_intact_area += intact_area
        ...
        logger.info(f"Analysis complete: {num_leaves} leaves, avg damage {avg_damage:.3f} "
                    f"({avg_damage*100:.1f}%)")
